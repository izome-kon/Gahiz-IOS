...
class NotificationRepository {
  Future<NotificationResponse> getNotifications({userID}) async {
    ...
    final response = await http.get(
      Uri.parse("${AppConfig.BASE_URL}/notifications/$userID"),
    );
    print(response.body);
  ...
  Future<List<Notification>> getUnReadNotifications({userID}) async {
    print(userID);
    final response = await http.get(
      Uri.parse("${AppConfig.BASE_URL}/notifications/unread/$userID"),
    );
  ...
  Future<void> setAsRead({userID}) async {
    print(userID);
    final response = await http.get(
      Uri.parse("${AppConfig.BASE_URL}/notifications/markAsRead/$userID"),
    );
  }
}
