...
class _LoginState extends State<Login> {
  ...
  void onLoginPressed() {
    ...
    if (_formKey.currentState.validate()) {
      ...
      AuthRepository()
          .getLoginResponse('$countryCode${phoneInt.toString()}', password)
          .then((v) async {
        print(v.result);
        if (v.result) {
          //authed
          ...
          freshchatUser.setPhone(
              '$countryCode', v.user.phone.substring(3, v.user.phone.length));
          Freshchat.setUser(freshchatUser);
          showTopSnackBar(
            context,
            CustomSnackBar.success(
              message: getLang(context, 'Welcome') + "Dr.${v.user.name} !",
  ...
  Future<void> onResetPassword() async {
    ...
    await _auth.verifyPhoneNumber(
      phoneNumber: countryCode + phoneInt.toString(),
  ...
  void onSubmitClick() {
    ...
    if (codeConfirmed) {
      ...
      AuthRepository()
          .getPasswordConfirmResponse(
              countryCode + phoneInt.toString(), password)
          .then((value) {
        if (value.result) {
          showTopSnackBar(
            context,
